...
function liquidchurch_entry_meta() {
	...
	if ( current_theme_supports( 'post-formats', $format ) ) {
		printf( '<span class="entry-format">%1$s<a href="%2$s">%3$s</a></span>',
			sprintf( '<span class="screen-reader-text">%s </span>', _x( 'Format', 'Used before post format.', 'liquidchurch' ) ),
	...
	if ( 'post' === get_post_type() ) {
		liquidchurch_entry_taxonomies();
	}

	if ( ! is_singular() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( sprintf( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'liquidchurch' ), get_the_title() ) );
		echo '</span>';
	}
}
endif;

if ( ! function_exists( 'liquidchurch_entry_date' ) ) :
/**
...
function liquidchurch_entry_date() {
	...
	if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
		$time_string = '<br /><time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
	}
	...
	printf( '<span class="posted-on"><span class="screen-reader-text">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span><br />',
		_x( 'Posted on', 'Used before publish date.', 'liquidchurch' ),
...
if ( ! function_exists( 'liquidchurch_entry_taxonomies' ) ) :
/**
...
function liquidchurch_entry_taxonomies() {
	...
	if ( $categories_list && liquidchurch_categorized_blog() ) {
		printf( '<span class="cat-links"><span class="screen-reader-text">%1$s </span>%2$s</span>',
			_x( 'Categories', 'Used before category names.', 'liquidchurch' ),
			$categories_list
		);
	}

	$tags_list = get_the_tag_list( '', _x( ', ', 'Used between list items, there is a space after the comma.', 'liquidchurch' ) );
	if ( $tags_list ) {
		printf( '<span class="tags-links"><span class="screen-reader-text">%1$s </span>%2$s</span>',
			_x( 'Tags', 'Used before tag names.', 'liquidchurch' ),
			$tags_list
		);
	}
}
...
if ( ! function_exists( 'liquidchurch_excerpt' ) ) :
	 ...
	 */
	function liquidchurch_excerpt( $class = 'entry-summary' ) {
...
function liquidchurch_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'liquidchurch_categories' ) ) ) {
		...
		set_transient( 'liquidchurch_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so liquidchurch_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so liquidchurch_categorized_blog should return false.
		return false;
	}
}
...
function liquidchurch_category_transient_flusher() {
	...
	// Like, beat it. Dig?
	delete_transient( 'liquidchurch_categories' );
}
add_action( 'edit_category', 'liquidchurch_category_transient_flusher' );
add_action( 'save_post',     'liquidchurch_category_transient_flusher' );

if ( ! function_exists( 'liquidchurch_the_custom_logo' ) ) :
/**
 ...
 */
function liquidchurch_the_custom_logo() {
