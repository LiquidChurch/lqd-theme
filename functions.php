...
function liquidchurch_setup() {
	...
	// Switch default core markup for search and comment forms to output valid HTML5.
	...
	/*
	 * TODO: This theme DOESN'T style the visual editor to resemble the theme style, but it should,
...
function liquidchurch_scripts() {
    // Ensure version of CSS styles is updated, helps with cache busting.
	...
	wp_enqueue_style( 'selectric-css', get_template_directory_uri() . '/css/selectric.css', [ 'liquidchurch-style' ],
    '20160412' );
...
/**
 * Adds preformatted tags around $obj.
 *
 * @param $obj
 */
function pr($obj){
...
function my_mce4_options($init) {
  ...
  // enable 6th row for custom colors in grid
...
function lqd_app_view_taxonomy_series_template( $template ) {
	...
	if( isset( $wp_query->query_vars['messages-app-view'] ) && isset( $wp_query->query_vars['gc-sermon-series'] ) ) {
		$template = locate_template( ['taxonomy-gc-sermon-series-app-view.php'] );
	}
	return $template;
}
...
function lqd_app_view_message_template( $template ) {
	...
	if( isset( $wp_query->query_vars['messages-app-view'] ) && isset( $wp_query->query_vars['gc-sermons'] ) ) {
		$template = locate_template( ['single-gc-sermons-app-view.php'] );
	}
	return $template;
}
